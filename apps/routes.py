...
from apps.models import Table, User, Item, Order
from apps.forms import RegisterForm, LoginForm, OrderIDForm, ReserveForm, AddForm, OrderForm
from apps import db
...
def cart_page():
    ...
    if request.method == 'POST':
        ...
        order_info = Order(user_id = current_user.id,  address=current_user.address
                           ) 
...
def track_page():
    ...
    # if request.method == "POST":
    if order_id.validate_on_submit:
        #check to see if order id matches
        valid_orderid = Order.query.filter_by( order_id = order_id.order_id.data ).first()
        if valid_orderid:
            return redirect(url_for('delivery'))
        else:
            flash('Your Order-ID is invalid! Please Try Again.', category = 'danger')

    return render_template('order-id.html', order_id = order_id)
